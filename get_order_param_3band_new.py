...
def get_order_param_3band_UHF(GRhoImp, AFM_idx = [0, 3, 9, 6]):
    limp = GRhoImp.shape[0] / 2
...
def get_order_param_3band(GRhoImp, AFM_idx = [0, 3, 9, 6], plot=None, \
        name=None, doping=None):
    cwd = os.getcwd().split('/')
    if name is None:
        name = cwd[-2]
    if doping is None:
        doping = cwd[-1].replace(".", "")
    ...
    if plot is not None:
        plt.savefig('%s-%s-%s.png'%(plot, name, doping), bbox_inches='tight', dpi=400)
        return None, None
    else:
...
if __name__ == '__main__':
    ...
    for plot in plots:
        print (get_order_param_3band(GRhoImp, plot=plot))
    #try:
    #    print get_order_param_3band(GRhoImp)
    #except:
    #    print get_order_param_3band_UHF(GRhoImp)
